PROGRAM SCOOTER
  ...
  freq0 = freqVec( Nfreq )   ! save the reference frequency for scaling the grid
  
  ! Set up vector of wavenumber samples
  kMin = REAL( 2 * pi * freq0 / cHigh )
  kMAX = REAL( 2 * pi * freq0 / cLow  )
  ...
  cVec   = 2 * pi * freq0 / k   ! phase speeds associated with wavenumbers
  ...
  FreqLoop: DO ifreq = 1, Nfreq
     ...
     CALL UpdateSSPLoss( freq )
     ! update loss in the halfspaces based onthe frequency
     ! depth of 1e20 is a large number to ensure bio loss is not included
     HSTop%cP  = CRCI( 1e20, HSTop%alphaR, HSTop%alphaI, freq, SSP%AttenUnit, bio )
     HSTop%cS  = CRCI( 1e20, HSTop%betaR,  HSTop%betaI,  freq, SSP%AttenUnit, bio )
     HSBot%cP  = CRCI( 1e20, HSBot%alphaR, HSBot%alphaI, freq, SSP%AttenUnit, bio )
     HSBot%cS  = CRCI( 1e20, HSBot%betaR,  HSBot%betaI,  freq, SSP%AttenUnit, bio )
