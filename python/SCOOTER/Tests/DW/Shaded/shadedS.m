...
disp('SCOOTER - shaded point source in free space') 

fid = fopen('shadedS.sbp','r');
n = fscanf(fid,'%d\n',[1 1]);
data = fscanf(fid,'%f %f\n',[2 n]);
fclose(fid);
...
[ PlotTitle, PlotType, freq, atten, Pos, p ] = read_shd( 'shadedS.shd' );
...
xlabel('Range (km)')
ylabel('Depth (m)')
title('SCOOTER - shaded point source in free space, 100 Hz')
