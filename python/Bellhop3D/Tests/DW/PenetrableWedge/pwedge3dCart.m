...
disp('Bellhop3D - Penetrable Wedge:') 
disp('Coherent TL calculation')
disp('Geometric hat beams in Cartesian coordinates')

system('bellhop3d.exe pwedge3dCart');

[ PlotTitle, PlotType, freq, atten, Pos, p ] = read_shd( 'pwedge3dCart.shd' );
...
pcolor(Xkm,Ykm,tl), shading interp, colormap( tej ), caxis([50 150]), 
h = colorbar;
set( h, 'YDir', 'reverse' );
hold off
xlabel('X (km)')
ylabel('Y (km)')
title('Bellhop3D - Penetrable Wedge')
