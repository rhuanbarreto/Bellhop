SUBROUTINE ReadEnvironment( FileRoot, ThreeD )
  ...
  ! Beam%Type( 1 : 1 ) is
  !   'G' or '^' for Geometric hat beams in Cartesian coordinates
  !   'g' for Geometric hat beams in ray-centered coordinates
  !   'B' for Geometric Gaussian beams
  !   'S' for Simple Gaussian beams
  !   'C' for Cerveny Gaussian beams in Cartesian coordinates
  !   'R' for Cerveny Gaussian beams in Ray-centered coordinates
  ...
  CASE ( 'G', 'g' , '^', 'B', 'S' )   ! geometric hat beams, geometric Gaussian beams, or simple Gaussian beams
...
SUBROUTINE ReadTopOpt( TopOpt, BC, AttenUnit, FileRoot )
  ...
  CASE ( 'B' )
     ...
     READ(  ENVFile, * ) bio%Z1, bio%Z2, bio%f0, bio%Q, bio%a0
     WRITE( PRTFile, * ) '      Top    of layer = ', bio%Z1, ' m'
     WRITE( PRTFile, * ) '      Bottom of layer = ', bio%Z2, ' m'
     WRITE( PRTFile, * ) '      Resonance frequency = ', bio%f0, ' Hz'
     WRITE( PRTFile, * ) '      Q  = ', bio%Q
     WRITE( PRTFile, * ) '      a0 = ', bio%a0
  CASE ( ' ' )
  CASE DEFAULT
