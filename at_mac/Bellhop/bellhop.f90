...
SUBROUTINE BellhopCore
  ...
  IF ( atiType( 2 : 2 ) == 'L' ) THEN
     DO iSeg = 1, NatiPts
        Top( iSeg )%HS%cp = CRCI( 1e20, Top( iSeg )%HS%alphaR, Top( iSeg )%HS%alphaI, freq, 'W ', bio )   ! compressional wave speed
        Top( iSeg )%HS%cs = CRCI( 1e20, Top( iSeg )%HS%betaR,  Top( iSeg )%HS%betaI,  freq, 'W ', bio )   ! shear         wave speed
     END DO
  END IF

  IF ( btyType( 2 : 2 ) == 'L' ) THEN
     DO iSeg = 1, NbtyPts
        Bot( iSeg )%HS%cp = CRCI( 1e20, Bot( iSeg )%HS%alphaR, Bot( iSeg )%HS%alphaI, freq, 'W ', bio )   ! compressional wave speed
        Bot( iSeg )%HS%cs = CRCI( 1e20, Bot( iSeg )%HS%betaR,  Bot( iSeg )%HS%betaI,  freq, 'W ', bio )   ! shear         wave speed
     END DO
  END IF
  ...
  SourceDepth: DO is = 1, Pos%Nsd
     ...
     ElevationAngle: DO ialpha = 1, Angles%Nalpha
        ...
        !IF ( ialpha == 50 ) THEN    ! Single beam run?
           ...
           IF ( Beam%RunType( 1 : 1 ) == 'S' ) Amp0 = SQRT( 2.0 ) * ABS( SIN( omega / c * xs( 2 ) * SIN( Angles%alpha( ialpha ) ) ))
...
SUBROUTINE TraceRay2D( xs, alpha, Amp0 )
  ...
  Stepping: DO istep = 1, MaxN - 1
     ...
     CALL Step2D( ray2D( is ), ray2D( is1 ),  &
          ...
          Bot( IsegBot )%x, Bot( IsegBot )%n, rTopSeg, rBotSeg )
...
SUBROUTINE Step2D( ray0, ray2, Topx, Topn, Botx, Botn, rTopSeg, rBotSeg )
  ...
  REAL (KIND=8 ), INTENT( IN ) :: Topx( 2 ), Topn( 2 ), Botx( 2 ), Botn( 2 ), rTopSeg( 2 ), rBotSeg( 2 )
  ...
  CALL ReduceStep2D( ray0%x, urayt0, iSegz0, iSegr0, Topx, Topn, Botx, Botn, rTopSeg, rBotSeg, Beam%deltas, h ) ! reduce h to land on boundary
  ...
  CALL ReduceStep2D( ray0%x, urayt1, iSegz0, iSegr0, Topx, Topn, Botx, Botn, rTopSeg, rBotSeg, Beam%deltas, h ) ! reduce h to land on boundary
...
SUBROUTINE ReduceStep2D( x0, urayt, iSegz0, iSegr0, Topx, Topn, Botx, Botn, rTopSeg, rBotSeg, deltas, h )
  ...
  REAL (KIND=8), INTENT( IN  ) :: rTopSeg( 2 ), rBotSeg( 2 )                 ! range intervals for top and bottom segments
