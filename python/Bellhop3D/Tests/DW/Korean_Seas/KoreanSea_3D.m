...
system('cp KoreanSea.bty KoreanSea_3D.bty');
system('cp KoreanSea.ssp KoreanSea_3D.ssp');

disp('Bellhop3D - Korean Sea 3D (3D run):') 
disp('Coherent TL calculation') 
disp('Geometric hat beams in Cartesian coordinates')

system('bellhop3d.exe KoreanSea_3D');
system('rm KoreanSea_3D.bty');
system('rm KoreanSea_3D.ssp');

[ PlotTitle, PlotType, freq, atten, Pos, p ] = read_shd( 'KoreanSea_3D.shd' );
...
xlabel('Range (km)')
ylabel('Depth (m)')
title('Bellhop3D - Korean Sea 3D (3D run)')
view(0,-90)
