...
if ( nargin == 1 )
    ...
    rr = 1000 * rr_km;
    rr( rr < realmin ) = 1;   % replace zero range with 1 m to avoid singularity
    % fprintf( '\n\n--- FieldSCO --- \n\nRminkm = %d, Rmaxkm = %d, Nrr = %i \n', Rminkm, Rmaxkm, Nrr )
    
    if ( min( rr ) < 0 )
        %warning( 'Rmin <0, G(-r) defined as G(r) if r<0' )
        error( 'Receive ranges must be nonnegative' )
    end
    
    fclose( fid );
    
    % read in the Green's function
    [ ~, ~, freqVec, ~, Pos, ~ ] = read_shd( filename );
...
pressure = zeros( Nfreq, Nsd, Nrd, Nrr );
...
for ifreq = 1 : Nfreq
    ...
    [ PlotTitle, ~, freqVec, atten, Pos, Gtemp ] = read_shd( filename, freqVec( ifreq ) );
    
    cVec = Pos.r.range;   % the phase speeds for the wavenumbers are stored in Pos.r.range
    k    = 2 * pi * freq ./ cVec;
    Nk   = length( k );              % # of wavenumbers
    
    deltak = ( k( end ) - k( 1 ) ) / ( length( k ) - 1 );
    
    if ( PlotTitle( 1 : 5 ) ~= 'SPARC' )
       atten  = deltak;   % this is calculated here because SCOOTER doesn't have different values for each frequency
    end
    
    ck     = k + 1i * atten;

    x  = ck * rr;   % this is a matrix of size nk * nrr
    ...
    for isd = 1: Nsd
        ...
        pressure( ifreq, isd, :, : ) = scalecol( Y, factor2 );  % cylindrical spreading, etc.
        %figure; imagesc( rr, Pos.r.depth, 20 * log10( abs( squeeze( pressure( ifreq, 1, :, : ) ) ) ) )
