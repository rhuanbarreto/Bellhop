...
disp('Scooter - Free space, line source') 

system('scooter.exe freeSLine');
system('fields.exe freeSLine < freeSLine.flp');

[ PlotTitle, PlotType, freq, atten, Pos, p ] = read_shd( 'freeSLine.shd' );
...
pcolor(rarraykm,zarray,tl), shading interp, colormap( tej ), caxis([10 20]), 
h = colorbar;
set( h, 'YDir', 'reverse' );
...
xlabel('Range (km)')
ylabel('Depth (m)')
title('SCOOTER - Free space, line source, 5 Hz')
